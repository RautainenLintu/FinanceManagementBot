...
class Account:
    ...
    def checkFundsSufficiency(self, sum):
        ...
        cursor = conn.cursor()
        cursor.execute(
            '''CREATE TABLE IF NOT EXISTS accounts (account_id INTEGER PRIMARY KEY, telegram_id INTEGER, balance DOUBLE)''')
        cursor.execute(
    ...
    def createAccountRecord(self, init_balance):
        ...
        conn = sqlite3.connect('./database.db')
        cursor = conn.cursor()
        cursor.execute('''CREATE TABLE IF NOT EXISTS accounts (account_id INTEGER PRIMARY KEY, balance DOUBLE)''')
